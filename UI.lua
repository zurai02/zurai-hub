--[[
	...
	Version 1.6.0 — adds Tab:CreateConfigManager(), a visible Save/Load
	UI (buttons + copyable text boxes) built on top of the existing
	ExportConfig/ImportConfig functions, which previously had no UI
	unless you wired one up yourself.
]]
...
function NovaUI:CreateWindow(config)
	...
	function Window:CreateTab(name, icon)
		...
		-- A visible Save/Load UI on top of NovaUI:ExportConfig()/ImportConfig().
		-- Those two functions only move data; this gives the player something
		-- to actually click. No clipboard access (that's an executor-only
		-- global, not a vanilla Roblox API) — the exported string is shown in
		-- a selectable multi-line box the player copies manually.
		function Tab:CreateConfigManager()
			...
			AddTopHighlight(saveBtn)
			Hover(saveBtn, theme.Elevated, Lighten(theme.Elevated))

			local exportBox = Create("TextBox", {
				Text = "",
				PlaceholderText = "Click \"Save Config\" to generate a string here, then select all and copy it.",
				Font = Enum.Font.Code,
				TextSize = 12,
				TextColor3 = theme.SubText,
			...
			saveBtn.MouseButton1Click:Connect(function()
				...
				if exported then
					exportBox.Text = exported
					exportBox.Visible = true
					NovaUI:Notify({ Title = "Config", Type = "Success", Content = "Saved. Select all in the box below and copy it.", Duration = 4 })
				else
					NovaUI:Notify({ Title = "Config", Type = "Danger", Content = "Nothing to save yet.", Duration = 4 })
				end
			end)
			...
			local importBox = Create("TextBox", {
				Text = "",
				PlaceholderText = "Paste a saved config string here",
				Font = Enum.Font.Code,
				TextSize = 12,
				TextColor3 = theme.Text,
				PlaceholderColor3 = theme.SubText,
			...
			AddTopHighlight(loadBtn)
			Hover(loadBtn, theme.Elevated, Lighten(theme.Elevated))

			loadBtn.MouseButton1Click:Connect(function()
				if importBox.Text == "" then
					NovaUI:Notify({ Title = "Config", Type = "Danger", Content = "Paste a config string first.", Duration = 4 })
					return
				end
				local ok = NovaUI:ImportConfig(importBox.Text)
				if ok then
					NovaUI:Notify({ Title = "Config", Type = "Success", Content = "Loaded. Values are in NovaUI.Flags — call each component's Set() to reflect them visually.", Duration = 5 })
				else
					NovaUI:Notify({ Title = "Config", Type = "Danger", Content = "That string couldn't be read.", Duration = 4 })
				end
			end)
		end
