...
local Window = Fluent:CreateWindow({
    Title="zurai hub", SubTitle="welcome", TabWidth=100,
    Size=UDim2.fromOffset(440,315), Acrylic=false,
    Theme="Darker", MinimizeKey=Enum.KeyCode.LeftControl
})
...
-- Services (single declarations, no duplicates)
...
-- Cached ping stat (avoids repeated path traversal)
...
local System = {
    __properties = {
        ...
        __play_animation=false, __curve_mode=1, __accuracy=1,
        __divisor_multiplier=1.1, __parried=false, __training_parried=false,
        __spam_threshold=1.5, __parries=0, __parry_key=nil,
        __grab_animation=nil, __tornado_time=tick(),
        __first_parry_done=false, __connections={}, __reverted_remotes={},
        __spam_accumulator=0, __spam_rate=240,
...
local function update_randomized_accuracy()
    ...
    local ping = tonumber(PingStat:GetValueString():match("%d+")) or 0
    local new_accuracy
...
task.spawn(function()
    ...
    ParryRemote = Network.RemoteEvent(Network, RemoteName)
end)
...
-- ========== PARRY FIRE ==========
local function buildPlayerPositions()
    local t = {}
    for _, entity in pairs(Alive:GetChildren()) do
        if entity:FindFirstChild("HumanoidRootPart") then
            t[entity.Name] = entity.HumanoidRootPart.Position
        end
    end
    return t
end
...
local function Parry()
    ...
    ParryRemote:FireServer(Hash1, Hash2, Hash3(), 0.025, Camera.CFrame, buildPlayerPositions(), {center.X, center.Y}, false)
end
...
-- Shared parry-count helper
local function tickParryCount()
...
function System.animation.play_grab_parry()
    ...
    local sword_name = getgenv().skinChangerEnabled and getgenv().swordAnimations
                       or character:GetAttribute('CurrentlyEquippedSword')
    ...
    if sword_data and sword_data['AnimationType'] then
        for _, object in pairs(sword_api:GetChildren()) do
            if object.Name == sword_data['AnimationType'] then
                local anim = object:FindFirstChild('GrabParry') or object:FindFirstChild('Grab')
                if anim then parry_animation = anim end
                break
            end
        end
    end
...
local function SpamParry_Animation()
    if (os.clock()-lastplayed) >= (AnimationDelay-0.8) or bypasscd then
        lastplayed = os.clock(); bypasscd = false
...
function System.player.get_closest()
    ...
    local max_distance = math.huge; local closest = nil
    if Alive then
        for _, entity in pairs(Alive:GetChildren()) do
            if entity ~= LocalPlayer.Character and entity.PrimaryPart then
                ...
                if d < max_distance then max_distance=d; closest=entity end
            end
        end
    end
...
function System.player.get_closest_to_cursor()
    ...
    local closest = nil; local minimal_dot = -math.huge
    ...
    for _, player in pairs(Alive:GetChildren()) do
        ...
        if dot > minimal_dot then minimal_dot=dot; closest=player end
    end
    return closest
end
...
function System.parry.execute()
    ...
    Parry(); tickParryCount()
end
function System.parry.keypress()
    if System.__properties.__parries > 10000 or not LocalPlayer.Character then return end
    fireParry(); tickParryCount()
end
...
Runtime.ChildAdded:Connect(function(Object)
    ...
    local Weld = Object:FindFirstChildWhichIsA("WeldConstraint"); if not Weld then return end
    local Character = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
    if not (Character and Weld.Part1==Character.HumanoidRootPart) then return end
...
ReplicatedStorage.Remotes.ParrySuccessAll.OnClientEvent:Connect(function(_,root)
    ...
    local char = LocalPlayer.Character
    local target_distance = (char.PrimaryPart.Position-closest.PrimaryPart.Position).Magnitude
    ...
    if target_distance < 15 and distance < 15 and dot > -0.25 and curve_detected then
...
ReplicatedStorage.Remotes.ParrySuccessAll.OnClientEvent:Connect(function(_,b)
    ...
    local Dot      = (char.PrimaryPart.Position-Ball.Position).Unit:Dot(Velocity.Unit)
    local Pings    = PingStat:GetValue()
...
function System.triggerbot.trigger(ball)
    ...
    local char = LocalPlayer.Character
    if char and char.PrimaryPart and char.PrimaryPart:FindFirstChild('SingularityCape') then return end
...
local macroFrameFireCount = 0; local macroFrameTime = 0; local macroRealCPS = 0
local macroAnimFix = true
...
-- Single Heartbeat handles spam firing (removed redundant coroutine)
RunService.Heartbeat:Connect(function(dt)
    ...
    if macroSpamActive and ParryRemote then
        pcall(fireParry); macroFrameFireCount = macroFrameFireCount + 1
...
function System.auto_spam:get_ball_properties()
    local ball = System.ball.get(); if not ball then return false end
    local char = LocalPlayer.Character; if not char or not char.PrimaryPart then return false end
    local diff = char.PrimaryPart.Position - ball.Position
...
function System.auto_spam.spam_service(self)
    ...
    local velocity = ball.AssemblyLinearVelocity or Vector3.zero; local speed = velocity.Magnitude
...
function System.auto_spam.start()
    ...
    local last_auto_spam=0; local last_target_check=0
    System.__properties.__connections.__auto_spam_connection = RunService.Heartbeat:Connect(function()
        ...
        if now-last_target_check > 0.1 then
            System.player.get_closest(); last_target_check = now
            if System.__properties.__spam_target then
                local t = System.__properties.__spam_target
                if not t.Parent or not t:FindFirstChild("Humanoid") or t.Humanoid.Health <= 0 then
        ...
        if not (ball_props and entity_props) then return end
        local ping = PingStat:GetValue()
        local ping_threshold = math.clamp(ping/5,1,16)
        local spam_accuracy = System.auto_spam.spam_service({
            Ball_Properties=ball_props, Entity_Properties=entity_props, Ping=ping_threshold
        })
        ...
        local target_dist  = (root.Position-target_entity.PrimaryPart.Position).Magnitude
        local dist_to_ball = (root.Position-ball.Position).Magnitude
        local spam_target  = System.__properties.__spam_target
        if not spam_target then return end
        if ball_target~=spam_target.Name and ball_target~=LocalPlayer.Name then return end
        if char:GetAttribute('Pulsed') then return end
        if target_dist > spam_accuracy or dist_to_ball > spam_accuracy then return end
        ...
        if target_dist > final_max_dist or dist_to_ball > final_max_dist then return end
        if System.__properties.__parries > System.__properties.__spam_threshold then
            if getgenv().AutoSpamMode=="Keypress" then System.parry.keypress()
            else
                System.parry.execute()
                if getgenv().AutoSpamAnimationFix then System.animation.play_grab_parry(); SpamParry_Animation() end
            end
        end
    end)
end
...
function System.autoparry.start()
    ...
    System.__properties.__connections.__autoparry = RunService.PreSimulation:Connect(function()
        if not System.__properties.__autoparry_enabled or not LocalPlayer.Character
        ...
        for _, ball in pairs(balls) do
            ...
            local char        = LocalPlayer.Character
            local distance    = (char.PrimaryPart.Position-ball.Position).Magnitude
            local ping        = PingStat:GetValue()/10
            ...
            if char.PrimaryPart:FindFirstChild('SingularityCape') then continue end
            ...
            if ball_target==LocalPlayer.Name and distance <= parry_accuracy then
                if getgenv().AutoAbility then
                    ...
                    if AbilityCD and AbilityCD.Offset.Y==0.5 then
                        ...
                        if abilities then
                            local abilityList = {"Raging Deflection","Rapture","Calming Deflection","Aerodynamic Slash","Fracture","Death Slash"}
                            for _, name in ipairs(abilityList) do
...
local function destroy_mobile_gui(g) if g and g.gui then g.gui:Destroy() end end
...
local function __av_set(name,char)
    ...
    local ok,desc=pcall(function()
        return Players:GetHumanoidDescriptionFromUserId(Players:GetUserIdFromNameAsync(name))
    end)
    ...
    task.spawn(function()
        ...
        pcall(function()
            ...
            if swordsController.currentSword~=nil then
                swordsController.currentSword = getgenv().swordFX~="" and getgenv().swordFX or getgenv().swordModel
            end
            if swordsController.SwordFX~=nil then
                swordsController.SwordFX = getgenv().swordFX~="" and getgenv().swordFX or getgenv().swordModel
            end
        end)
    end)
end
...
autoparry_section:AddToggle("AutoParryToggle",{
    Title="Auto Parry", Description="Automatically parries ball", Default=false,
...
autoparry_section:AddDropdown("ParryMode",{Title="Parry Mode",Values={"Remote","Keypress"},Default="Remote",Multi=false,Callback=function(v) getgenv().AutoParryMode=v end})
autoparry_section:AddDropdown("ModeCurve",{Title="Mode curve",Values=System.__config.__curve_names,Default="Camera",Multi=false,Callback=function(value)
    for i,name in ipairs(System.__config.__curve_names) do
        if name==value then System.__properties.__curve_mode=i; break end
    end
end})
autoparry_section:AddSlider("ParryAccuracy",{Title="Accuracy",Default=50,Min=1,Max=100,Rounding=1,Callback=function(v) System.__properties.__accuracy=v; update_divisor() end})
autoparry_section:AddToggle("RandomizeAccuracy",{Title="Randomize Accuracy",Default=false,Callback=function(v) System.__properties.__randomized_accuracy_enabled=v; if v then update_randomized_accuracy() end end})
autoparry_section:AddToggle("CooldownProtection",{Title="Cooldown Protection",Default=false,Callback=function(v) getgenv().CooldownProtection=v end})
autoparry_section:AddToggle("AutoAbility",{Title="Auto Ability",Default=false,Callback=function(v) getgenv().AutoAbility=v end})
autoparry_section:AddToggle("AutoParryNotify",{Title="Notify",Default=false,Callback=function(v) getgenv().AutoParryNotify=v end})
        ...
        if System.__properties.__is_mobile then
            if value then
                if not System.__properties.__mobile_guis.triggerbot then
                    local mb=create_mobile_button('Trigger',0.7,Color3.fromRGB(255,100,0))
                    System.__properties.__mobile_guis.triggerbot=mb
                    local ts=0; local wd=false
                    mb.button.InputBegan:Connect(function(input) if input.UserInputType==Enum.UserInputType.Touch then ts=tick(); wd=false end end)
                    mb.button.InputChanged:Connect(function(input) if input.UserInputType==Enum.UserInputType.Touch and (tick()-ts)>0.1 then wd=true end end)
                    mb.button.InputEnded:Connect(function(input)
                        if input.UserInputType==Enum.UserInputType.Touch and not wd then
                            ...
                            mb.text.Text=System.__properties.__triggerbot_enabled and "ON" or "Trigger"
                            mb.text.TextColor3=System.__properties.__triggerbot_enabled and Color3.fromRGB(255,100,0) or Color3.fromRGB(255,255,255)
                ...
                destroy_mobile_gui(System.__properties.__mobile_guis.triggerbot); System.__properties.__mobile_guis.triggerbot=nil
            end
        else
...
triggerbot_section:AddToggle("TriggerbotNotify",{Title="Notify",Default=false,Callback=function(v) getgenv().TriggerbotNotify=v end})

local autocurve_section=Tabs.Rage:AddSection("AutoCurve Hotkey","keyboard")
autocurve_section:AddToggle("AutoCurveHotkey",{Title="AutoCurve Hotkey",Default=false,Callback=function(v) getgenv().AutoCurveHotkeyEnabled=v end})
autocurve_section:AddToggle("AutoCurveHotkeyNotify",{Title="Notify",Default=false,Callback=function(v) getgenv().AutoCurveHotkeyNotify=v end})
        ...
        if System.__properties.__is_mobile then
            if state then
                if not System.__properties.__mobile_guis.manual_spam then
                    local mb=create_mobile_button('Spam',0.8,Color3.fromRGB(255,140,0))
                    System.__properties.__mobile_guis.manual_spam=mb
                    local ts=0; local wd=false
                    mb.button.InputBegan:Connect(function(input) if input.UserInputType==Enum.UserInputType.Touch then ts=tick(); wd=false end end)
                    mb.button.InputChanged:Connect(function(input) if input.UserInputType==Enum.UserInputType.Touch and (tick()-ts)>0.1 then wd=true end end)
                    mb.button.InputEnded:Connect(function(input)
                        if input.UserInputType==Enum.UserInputType.Touch and not wd then
                            ...
                            mb.text.Text=ns and "ON" or "Spam"
                            mb.text.TextColor3=ns and Color3.fromRGB(255,140,0) or Color3.fromRGB(255,255,255)
                ...
                destroy_mobile_gui(System.__properties.__mobile_guis.manual_spam); System.__properties.__mobile_guis.manual_spam=nil
            end
        else
...
manual_spam_section:AddToggle("ManualSpamNotify",{Title="Notify",Default=false,Callback=function(v) getgenv().ManualSpamNotify=v end})
manual_spam_section:AddDropdown("ManualSpamMode",{Title="Mode",Values={"Remote","Keypress"},Default="Remote",Multi=false,Callback=function(v) getgenv().ManualSpamMode=v end})
        ...
        if val then
            ...
            if not __av_flags['loop'] then
                __av_flags['loop']=LocalPlayer.CharacterAdded:Connect(function(char)
                    task.wait(.75); if __av_flags['name'] then __av_set(__av_flags['name'],char) end
                end)
            end
        else
...
avatar_section:AddInput("TargetUsername",{Title="Target Username",Placeholder="Enter Username...",Default="",Callback=function(val)
    __av_flags['name']=val
...
fov_section:AddToggle("FOV",{Title="FOV",Default=false,Callback=function(value)
...
fov_section:AddSlider("CameraFOV",{Title="Camera FOV",Default=70,Min=50,Max=120,Rounding=1,Callback=function(v)
    getgenv().CameraFOV=v; if getgenv().CameraEnabled then Camera.FieldOfView=v end
end})

local character_section=Tabs.Player:AddSection("Character","user")
character_section:AddToggle("CharacterModifier",{Title="Character Modifier",Default=false,Callback=function(value)
    ...
    if value then
        ...
        if getgenv().CharacterConnection then
            ...
            if char then
                ...
                if hum and getgenv().OriginalValues then
                    hum.WalkSpeed=getgenv().OriginalValues.WalkSpeed or 16
                    ...
                    hum.HipHeight=getgenv().OriginalValues.HipHeight or 0
                    hum.AutoRotate=getgenv().OriginalValues.AutoRotate or true
                end
            end
...
character_section:AddSlider("WalkSpeedValue",{Title="Walk Speed Value",Default=36,Min=16,Max=500,Rounding=1,Callback=function(v)
    ...
    if getgenv().CharacterModifierEnabled and getgenv().WalkspeedCheckboxEnabled then
        local char=LocalPlayer.Character; if char and char:FindFirstChild("Humanoid") then char.Humanoid.WalkSpeed=v end
    end
end})
...
character_section:AddSlider("JumpPowerValue",{Title="Jump Power Value",Default=50,Min=50,Max=200,Rounding=1,Callback=function(v)
    ...
    if getgenv().CharacterModifierEnabled and getgenv().JumpPowerCheckboxEnabled then
        local char=LocalPlayer.Character; if char and char:FindFirstChild("Humanoid") then
            local hum=char.Humanoid
...
character_section:AddSlider("HipHeightValue",{Title="Hip Height Value",Default=0,Min=-5,Max=20,Rounding=1,Callback=function(v)
    ...
    if getgenv().CharacterModifierEnabled and getgenv().HipHeightCheckboxEnabled then
        local char=LocalPlayer.Character; if char and char:FindFirstChild("Humanoid") then char.Humanoid.HipHeight=v end
    end
end})
...
function ability_esp.create_billboard(player)
    ...
    local head=character:FindFirstChild("Head"); if not head then return nil end
...
function ability_esp.setup_character(player)
    task.wait(0.1); local label,billboard=ability_esp.create_billboard(player); if not label then return end
...
function ability_esp.start()
    ...
    for _,player in pairs(Players:GetPlayers()) do ability_esp.add_player(player) end
...
function System.create_speed_gui()
    ...
    speedLabel.BackgroundTransparency=1; speedLabel.Text="Ball Velocity: 0.0"  -- typo fixed
...
function System.update_speed_display()
    ...
    speedLabel.Text = zoomies and string.format("Speed: %.1f",zoomies.VectorVelocity.Magnitude) or "Speed: 0.0"
end
...
skin_changer_section:AddToggle("SkinChanger",{Title="Skin Changer",Description="Change sword skins",Default=false,Callback=function(value)
    getgenv().skinChangerEnabled=value
    Fluent:Notify({Title="Skin Changer",Content=value and "Enabled" or "Disabled",Duration=2})
    if value then getgenv().updateSword() end
end})
skin_changer_section:AddToggle("ChangeSwordModel",{Title="Change Sword Model",Default=true,Callback=function(v) getgenv().changeSwordModel=v; if getgenv().skinChangerEnabled then getgenv().updateSword() end end})
skin_changer_section:AddInput("SwordModelName",{Title="Sword Model Name",Placeholder="Enter Sword Model Name...",Default="",Callback=function(t) getgenv().swordModel=t; if getgenv().skinChangerEnabled and getgenv().changeSwordModel then getgenv().updateSword() end end})
skin_changer_section:AddToggle("ChangeSwordAnimation",{Title="Change Sword Animation",Default=true,Callback=function(v) getgenv().changeSwordAnimation=v; if getgenv().skinChangerEnabled then getgenv().updateSword() end end})
skin_changer_section:AddInput("SwordAnimationName",{Title="Sword Animation Name",Placeholder="Enter Sword Animation Name...",Default="",Callback=function(t) getgenv().swordAnimations=t; if getgenv().skinChangerEnabled and getgenv().changeSwordAnimation then getgenv().updateSword() end end})
skin_changer_section:AddToggle("ChangeSwordFX",{Title="Change Sword FX",Default=true,Callback=function(v) getgenv().changeSwordFX=v; if getgenv().skinChangerEnabled then getgenv().updateSword() end end})
skin_changer_section:AddInput("SwordFXName",{Title="Sword FX Name",Placeholder="Enter Sword FX Name...",Default="",Callback=function(t) getgenv().swordFX=t; if getgenv().skinChangerEnabled and getgenv().changeSwordFX then getgenv().updateSword() end end})
...
if System.__properties.__is_mobile then
    task.spawn(function()
        task.wait(2)
        local gui=Instance.new('ScreenGui'); gui.Name='zuraiMobileUIButton'; gui.ResetOnSpawn=false
...
keybinds_section:AddKeybind("SpamKeybind",{Title="Spam Key",Description="",Default="E",Callback=function() end,
    ChangedCallback=function(k)
        local kc=Enum.KeyCode[k]; if kc then spamKeyCode=kc; Fluent:Notify({Title="Spam Key Set",Content=k,Duration=2}) end
    end})
keybinds_section:AddKeybind("TriggerKeybind",{Title="Trigger Key",Description="",Default="R",Callback=function() end,
    ChangedCallback=function(k)
        local kc=Enum.KeyCode[k]; if kc then trigKeyCode=kc; Fluent:Notify({Title="Trigger Key Set",Content=k,Duration=2}) end
    end})

SaveManager:SetLibrary(Fluent)
InterfaceManager:SetLibrary(Fluent)
SaveManager:IgnoreThemeSettings()
InterfaceManager:SetFolder("zurai hub")
SaveManager:SetFolder("zurai hub")
InterfaceManager:BuildInterfaceSection(Tabs.Settings)
SaveManager:BuildConfigSection(Tabs.Settings)
Window:SelectTab(1)
SaveManager:LoadAutoloadConfig()

-- ========== SINGLE MERGED INPUT HANDLER ==========
-- Handles both keyboard and gamepad in one connection
UserInputService.InputBegan:Connect(function(input, gp)
    ...
    if kc == spamKeyCode then
        ...
        macroSpamActive = ns
        if ns then System.manual_spam.start() else System.manual_spam.stop() end
        if getgenv().ManualSpamNotify then Fluent:Notify({Title="Manual Spam",Content=ns and "ON" or "OFF",Duration=1}) end
    elseif kc == trigKeyCode then
