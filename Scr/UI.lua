--[[
	...
	ARCHITECTURE
	  • Single shared UIS InputChanged/InputEnded pair per window (not one per
	    component). All sliders, color pickers, and resize use beginDrag() to
	    register the active handler; the window dispatches to it. Eliminates
	    O(n) redundant global connections scaling with component count.
	  • onTheme(fn) listener system is now actually called from every component
	    that owns color-bearing surfaces, so Window:SetTheme() repaints the
	    entire UI at runtime — not just the structural shell.
	  • Debounce() utility is used for slider callbacks: fires immediately on
	    mousedown, throttled during drag, then fires once more on release with
	    the final quantized value.
	  • zi is now captured once per component (local z = zi()) instead of
	    calling zi() repeatedly inside the same component body, preventing
	    ZIndex drift when Page scrolls mid-render.
	  • Window:Destroy() cleanly disconnects every RBXScriptConnection,
	    including per-component spinner/shimmer heartbeats.
	  • NovaUI:SetGlobalTheme(name) — updates all open windows at once.
	  • Window:GetTab(name), Window:RemoveTab(name) — tab management API.
	  • Tab:GetApi(flag) — retrieve the {Set,Get} object for any flagged
	    component after creation.

	BUGS FIXED (new in 3.2.0)
	  • CreateNumberInput: buttons were given wrong Position after creation
	    (.Position reassignment post-Create); now set correctly inside Create().
	  • CreateChipGroup: UIGridLayout CellSize(0,0,…) produced zero-width cells
	    on some Roblox clients; replaced with UIListLayout + Wraps=true.
	  • CreateAccordion: AbsoluteSize read inside task.defer could still be 0
	    if content hadn't laid out; now uses a HeartbeatStep loop to wait for
	    a non-zero measurement before tweening.
	  • CreateSkeleton shimmer: gradient Offset drifted to ±∞ after long
	    sessions; now uses tick()%period so it stays bounded forever.
	  • CreateTabGroup: all page frames were in the same parent with
	    AutomaticSize, causing all content to stack. Each page is now a
	    separate Visible=false child, properly hidden by selectInline().
	  • CreateDropdown outside-click: AbsolutePosition could read (0,0) before
	    the holder rendered; detection is now deferred one frame.
	  • CreateSpinner.Start(): re-calling Start() added a new connection to the
	    global _conns table each time (leaking). Now replaces the old one.
	  • Notification dismiss: wrapper height collapsed before the card finished
	    sliding out, causing a frame of visual overlap. Properly sequenced.
	  • CreateColorPicker Set(): hex label now updates alongside swatch and bars.
	  • Shadow does not snap back to window center after a drag; it now follows
	    Main.AbsolutePosition via a lightweight RenderStepped connection.

	NEW COMPONENTS
	  • Tab:CreateStepSlider(opts)    — snapping slider with labelled stops
	  • Tab:CreateTimeline(opts)      — vertical event log with type colours
	  • Tab:CreateList(opts)          — scrollable item list with icons + select
	  • Tab:CreateBanner(opts)        — hero/announcement card with bg image
	  • Tab:CreateCodeBlock(opts)     — monospace display with a copy button
	  • NovaUI:CreateDrawer(opts)     — slide-in side panel (window-level)
	  • NovaUI:CreateContextMenu(opts)— right-click floating menu (window-level)

	NEW THEMES
	  • "Midnight" — deep indigo/violet dark theme
	  • "Copper"   — warm amber-copper dark theme
...
local function Ripple(parent, clickPos, color)
	...
	-- clickPos may be nil on mobile if called without a position; default to centre
	local cx = clickPos and (clickPos.X - abs.X) or sz.X/2
	local cy = clickPos and (clickPos.Y - abs.Y) or sz.Y/2
...
local function Tooltip(parent, theme, text)
	...
	-- MouseEnter works on PC; InputBegan Touch works on mobile
	badge.MouseEnter:Connect(showTip)
	badge.MouseLeave:Connect(hideTip)
	-- Mobile tap: toggle tooltip visibility
	if UserInputService.TouchEnabled then
		badge.InputBegan:Connect(function(inp)
			if inp.UserInputType==Enum.UserInputType.Touch then
				if tip.Visible then hideTip() else showTip() end
			end
		end)
	end
end
...
Themes.Midnight = {
	...
	Secondary    = Color3.fromRGB(9, 11, 18),
	Elevated     = Color3.fromRGB(14, 16, 28),
	ElevatedHover= Color3.fromRGB(20, 23, 40),
	Accent       = Color3.fromRGB(142, 100, 255),
	AccentLight  = Color3.fromRGB(180, 148, 255),
	AccentDark   = Color3.fromRGB(100, 64, 210),
	AccentBg     = Color3.fromRGB(18, 10, 44),
	Text         = Color3.fromRGB(234, 232, 252),
	SubText      = Color3.fromRGB(130, 124, 162),
	MutedText    = Color3.fromRGB(64, 60, 92),
	Stroke       = Color3.fromRGB(26, 22, 48),
	StrokeStrong = Color3.fromRGB(38, 33, 68),
	Success      = Color3.fromRGB(88, 215, 140),  SuccessBg = Color3.fromRGB(8, 30, 18),
	Danger       = Color3.fromRGB(240, 80, 100),   DangerBg  = Color3.fromRGB(38, 8, 14),
	Warning      = Color3.fromRGB(238, 186, 50),   WarningBg = Color3.fromRGB(36, 26, 6),
	Info         = Color3.fromRGB(100, 164, 252),  InfoBg    = Color3.fromRGB(8, 18, 46),
	TooltipBg    = Color3.fromRGB(14, 12, 28),
	Tooltip      = Color3.fromRGB(38, 33, 68),
}
-- ── Copper (warm amber-copper) ────────────────────────────────────────── --
Themes.Copper = {
	Background   = Color3.fromRGB(16, 10, 8),
	Secondary    = Color3.fromRGB(24, 16, 12),
	Elevated     = Color3.fromRGB(34, 22, 16),
	ElevatedHover= Color3.fromRGB(44, 30, 22),
	Accent       = Color3.fromRGB(210, 118, 58),
	AccentLight  = Color3.fromRGB(240, 156, 90),
	AccentDark   = Color3.fromRGB(168, 86, 30),
	AccentBg     = Color3.fromRGB(40, 18, 8),
	Text         = Color3.fromRGB(248, 238, 228),
	SubText      = Color3.fromRGB(166, 130, 106),
	MutedText    = Color3.fromRGB(92, 64, 48),
	Stroke       = Color3.fromRGB(50, 30, 20),
	StrokeStrong = Color3.fromRGB(70, 44, 30),
	Success      = Color3.fromRGB(80, 210, 128),  SuccessBg = Color3.fromRGB(8, 28, 16),
	Danger       = Color3.fromRGB(236, 80, 74),    DangerBg  = Color3.fromRGB(40, 8, 8),
	Warning      = Color3.fromRGB(238, 186, 50),   WarningBg = Color3.fromRGB(36, 26, 6),
	Info         = Color3.fromRGB(90, 165, 248),   InfoBg    = Color3.fromRGB(8, 20, 44),
	TooltipBg    = Color3.fromRGB(28, 16, 10),
	Tooltip      = Color3.fromRGB(70, 44, 30),
}
...
-- Returns true when the game is running on a touch/mobile device.
-- Used internally to scale sizes, font, and hit-targets.
local function IsMobile()
	return UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled
end

-- Scale factor: 1.0 on PC, 1.15 on mobile (larger hit-targets, font).
local function UIS() return IsMobile() and 1.15 or 1.0 end

-- Clamp a pixel size with mobile scaling.
local function ScalePx(n) return math.floor(n * UIS() + 0.5) end

--// ──────────────────────────── KEY SYSTEM ────────────────────────────────── //
--[[
  NovaUI:CreateKeySystem(opts) — optional key gate before the window appears.

  opts = {
    Title       = "Key Required",
    Subtitle    = "Get your key at example.com",
    Keys        = { "MYKEY-123", "BETA-456" },  -- accepted key strings
    SaveKey     = true,                           -- persist via DataStore or attribute
    AttributeName = "NovaUIKey",                  -- LocalPlayer attribute name for save
    OnSuccess   = function(key) ... end,
    OnFail      = function(key) ... end,
    -- If Keys is nil/empty the system just shows a UI for the user to enter
    -- a key and calls OnSuccess with whatever they type (for server validation).
  }

  Returns:
    { Destroy = fn }        — the gate GUI; call Destroy() to close it.
  ...
  Example:
    NovaUI:CreateKeySystem({
      ...
      OnSuccess = function()
        ...
        local win = NovaUI:CreateWindow({ Title="Demo" })
        ...
      end,
    })
]]
function NovaUI:CreateKeySystem(opts)
	...
	local attrName    = opts.AttributeName or "NovaUIKey"

	-- Check for a previously saved key on the LocalPlayer attribute
	if saveKey then
		local saved = LocalPlayer:GetAttribute(attrName)
		if saved and type(saved)=="string" then
			local valid = #keys == 0  -- no keys = always valid
			for _, k in ipairs(keys) do
				if saved == k then valid=true; break end
			end
			if valid then
				if opts.OnSuccess then task.spawn(opts.OnSuccess, saved) end
	...
	local sg = Create("ScreenGui",{
		Name="NovaUI_KeySystem", ResetOnSpawn=false,
	...
	Create("Frame",{
		...
		BorderSizePixel=0, ZIndex=0, Parent=sg,
	})

	local W = IsMobile() and 320 or 380
	local card = Create("Frame",{
		Name="KeyCard",
		AnchorPoint=Vector2.new(0.5,0.5), Position=UDim2.new(0.5,0,0.5,0),
		Size=UDim2.fromOffset(W,0), AutomaticSize=Enum.AutomaticSize.Y,
	...
	-- Lock icon (pure text)
	Create("TextLabel",{
		Text="🔑", Font=Enum.Font.Gotham, TextSize=ScalePx(28),
		BackgroundTransparency=1, TextXAlignment=Enum.TextXAlignment.Center,
		Size=UDim2.new(1,0,0,36), ZIndex=2, Parent=card,
	})

	Create("TextLabel",{
		Text=title, Font=Enum.Font.GothamBold, TextSize=ScalePx(17),
	...
	Create("TextLabel",{
		Text=subtitle, Font=Enum.Font.Gotham, TextSize=ScalePx(12),
	...
	local inputHolder = Create("Frame",{
		BackgroundColor3=theme.Elevated, Size=UDim2.new(1,0,0,ScalePx(42)),
		ZIndex=2, Parent=card,
	},{
		Create("UICorner",{CornerRadius=UDim.new(0,12)}),
		Create("UIStroke",{Name="InputStroke",Color=theme.Stroke,Thickness=1}),
	})
	local keyBox = Create("TextBox",{
		Text="", PlaceholderText="Enter key here…",
		Font=Enum.Font.Code, TextSize=ScalePx(13),
		...
		ZIndex=3, Parent=inputHolder,
	})
	local inputStroke = inputHolder:FindFirstChild("InputStroke")
	keyBox.Focused:Connect(function()
		if inputStroke then Tween(inputStroke,{Color=theme.Accent,Thickness=1.5},0.14) end
	end)
	keyBox.FocusLost:Connect(function()
		if inputStroke then Tween(inputStroke,{Color=theme.Stroke,Thickness=1},0.14) end
	end)

	-- Status label (hidden until attempt)
	local statusLbl = Create("TextLabel",{
		Text="", Font=Enum.Font.GothamMedium, TextSize=ScalePx(12),
		TextColor3=theme.Danger, BackgroundTransparency=1,
		TextXAlignment=Enum.TextXAlignment.Center, TextWrapped=true,
		AutomaticSize=Enum.AutomaticSize.Y, Size=UDim2.new(1,0,0,0),
		Visible=false, ZIndex=2, Parent=card,
	})

	-- Submit button
	local submitBtn = Create("TextButton",{
		Text="Unlock", Font=Enum.Font.GothamBold, TextSize=ScalePx(14),
		TextColor3=Color3.new(0.06,0.06,0.06), BackgroundColor3=theme.Accent,
		Size=UDim2.new(1,0,0,ScalePx(40)), ZIndex=2, Parent=card,
	},{
		Create("UICorner",{CornerRadius=UDim.new(0,12)}),
		Create("UIGradient",{
			Color=ColorSequence.new(theme.AccentLight,theme.AccentDark), Rotation=90,
		}),
	})
	PressFeedback(submitBtn)

	-- Entrance animation
	card.Size = UDim2.fromOffset(W * 0.88, 0)
	Tween(card,{Size=UDim2.fromOffset(W,0),BackgroundTransparency=0},0.28,
	...
	local function tryKey(input)
		...
		attempts += 1
		local valid = #keys == 0  -- if no keys provided, accept anything
		for _, k in ipairs(keys) do
		...
		if valid then
			...
			statusLbl.Visible = true
			Tween(submitBtn,{BackgroundColor3=theme.Success},0.18)
			-- Save to LocalPlayer attribute
			if saveKey then
				pcall(function() LocalPlayer:SetAttribute(attrName, input) end)
			end
			task.wait(0.55)
			Tween(card,{BackgroundTransparency=1,Size=UDim2.fromOffset(W*0.9,0)},0.20)
			task.wait(0.22)
			sg:Destroy()
			if opts.OnSuccess then task.spawn(opts.OnSuccess, input) end
		else
			statusLbl.Text = ("✕  Invalid key. (attempt %d)"):format(attempts)
			statusLbl.TextColor3 = theme.Danger
			statusLbl.Visible = true
			-- Shake animation
			local orig = card.Position
			for i=1,4 do
				Tween(card,{Position=orig+UDim2.fromOffset(i%2==0 and 8 or -8,0)},0.05)
				task.wait(0.055)
			end
			Tween(card,{Position=orig},0.10)
			if opts.OnFail then task.spawn(opts.OnFail, input) end
		end
	end

	submitBtn.MouseButton1Click:Connect(function() tryKey(keyBox.Text) end)
	keyBox.FocusLost:Connect(function(enter) if enter then tryKey(keyBox.Text) end end)
...
function NovaUI:CreateWindow(config)
	...
	-- On mobile, default to the full viewport; on PC use a fixed 600×420 window.
	local mobile    = IsMobile()
	local vp        = workspace.CurrentCamera.ViewportSize
	local defSize   = mobile
		and UDim2.fromOffset(math.min(vp.X - 16, 480), math.min(vp.Y - 20, 560))
		or  UDim2.fromOffset(600, 420)
	local size      = config.Size      or defSize
	local toggleKey = config.ToggleKey or Enum.KeyCode.RightControl
	local initPos   = config.Position
		or (mobile
			and UDim2.fromOffset(8, 10)
	...
	-- Shadow follows Main position every frame so it stays centred after dragging.
	track(RunService.RenderStepped:Connect(function()
		if not Main or not Main.Parent then return end
		local ap = Main.AbsolutePosition
		local as = Main.AbsoluteSize
		Shadow.Position = UDim2.fromOffset(ap.X + as.X/2, ap.Y + as.Y/2 + 6)
		Shadow.Size     = UDim2.fromOffset(as.X + 36, as.Y + 36)
	end))

	-- ── Top bar ──────────────────────────────────────────────────────────
	-- Taller on mobile for easier touch targets
	local TopBarH = mobile and 58 or 50
	...
	-- Minimise + maximise — PC only; pointless on a touch-only screen
	if not mobile then
	...
	Create("TextLabel",{
		...
		Font=Enum.Font.GothamBold, TextSize=mobile and 17 or 15, TextColor3=theme.Text,
		TextXAlignment=Enum.TextXAlignment.Left, BackgroundTransparency=1,
		Position=UDim2.new(0,76,0,mobile and 10 or 8), Size=UDim2.new(1,-100,0,mobile and 22 or 19),
	...
	-- Dragging the window is only useful on PC; on mobile the window fills
	-- most of the screen and touch-drag would fight with scroll.
	if not mobile then
	...
	local ResizeHandle = Create("Frame",{
		...
		-- Hidden on mobile: no mouse cursor to grab it
		Visible=not mobile,
	...
	do
		local sPos, sSize
		local minSz = IsMobile() and Vector2.new(280,200) or Vector2.new(420,290)
	...
	-- ── Tab rail ──────────────────────────────────────────────────────────
	-- On mobile the rail is narrower (44px icon strip) to save screen space.
	local RAIL_W = mobile and 44 or 158
	...
	-- ── Notification holder ───────────────────────────────────────────────
	local notifyW = mobile and (workspace.CurrentCamera.ViewportSize.X - 24) or 300
	local NotifyHolder = Create("Frame",{
		Name="Notifications",
		AnchorPoint=Vector2.new(1,0), Position=UDim2.new(1,-12,0,12),
		Size=UDim2.new(0,notifyW,1,-24),
	...
	-- ── Window object ─────────────────────────────────────────────────────
	-- Component API registry: flag → {Set,Get} returned by each component
	local _components = {}
	...
	-- Return an existing Tab object by name.
	...
	-- Destroy a tab and its sidebar button. Selects the first remaining tab.
	...
	-- Retrieve the {Set, Get} API for any component registered with a Flag.
	function Window:GetApi(flag)
	...
	function Window:CreateTab(tabName, icon)
		local Page = Create("ScrollingFrame",{
			...
			ScrollBarThickness=mobile and 6 or 3, ScrollBarImageColor3=theme.Accent,
		...
		-- zi() returns a fresh z-index captured at the moment a component is created.
		-- Each component calls local z = zi() once and uses z throughout,
		-- preventing drift if Page.ZIndex changes mid-render.
		...
		function Tab:CreateSection(text)
			...
			Create("TextLabel",{
				...
				Size=UDim2.new(1,-4,0,18), ZIndex=zi(), Parent=holder,
			})
			Create("Frame",{
		...
		function Tab:CreateSlider(opts)
			...
			-- Invisible touch-hitbox behind the visual bar so mobile users can
			-- easily grab the slider without pixel-perfect precision.
			local sliderHit = Create("Frame",{
				Position=UDim2.new(0,14,0,30), Size=UDim2.new(1,-28,0,20),
				BackgroundTransparency=1, ZIndex=zi()+1, Parent=holder,
			})
			local sliderBar = Create("Frame",{
				AnchorPoint=Vector2.new(0,0.5), Position=UDim2.new(0,0,0.5,0),
				Size=UDim2.new(1,0,0,5),
				BackgroundColor3=theme.Stroke, ZIndex=zi()+1, Parent=sliderHit,
			},{Create("UICorner",{CornerRadius=UDim.new(1,0)})})
			...
			sliderHit.InputBegan:Connect(function(inp)
				if inp.UserInputType==Enum.UserInputType.MouseButton1 or
				   inp.UserInputType==Enum.UserInputType.Touch then
					...
					setAlpha((inp.Position.X-sliderHit.AbsolutePosition.X)/sliderHit.AbsoluteSize.X, true)
					beginDrag(function(mi)
						setAlpha((mi.Position.X-sliderHit.AbsolutePosition.X)/sliderHit.AbsoluteSize.X, true)
					end, function()
		...
		function Tab:CreateDropdown(opts)
			...
			local function rebuild()
				...
				for _, opt in ipairs(options) do
					...
					local row = Create("TextButton",{
						...
						Size=UDim2.new(1,0,0,mobile and 36 or 28), ZIndex=zi()+2, Parent=listHolder,
					},{
			...
			-- Close on outside click. We defer one frame so AbsolutePosition
			-- is valid (it reads (0,0) if queried in the same frame as creation).
			...
			head.MouseButton1Click:Connect(function()
				open = not open
				local rowPx = mobile and 36 or 30
				local maxRows = mobile and 5 or 8
				local h = open and (42+math.min(#options,maxRows)*rowPx+4) or 38
		...
		function Tab:CreateNumberInput(opts)
			...
			-- Value box (right side, centred between the two buttons)
			local box = Create("TextBox",{
				Text=tostring(val),
			...
			-- Minus button – position fixed inside Create() (no post-hoc reassignment)
			local minusBtn = Create("TextButton",{
				Text="−", Font=Enum.Font.GothamBold, TextSize=16,
				TextColor3=theme.Accent, BackgroundColor3=theme.AccentBg,
				AnchorPoint=Vector2.new(1,0.5), Position=UDim2.new(1,-127,0.5,0),
				Size=UDim2.new(0,28,0,28), ZIndex=zi()+1, Parent=holder,
			},{Create("UICorner",{CornerRadius=UDim.new(0,8)})})
			-- Plus button
			...
			PressFeedback(minusBtn); PressFeedback(plusBtn)
		...
		function Tab:CreateColorPicker(opts)
			...
			local function makeChan(lbl, initial, y, onChange)
				...
				-- Invisible touch-hit zone behind the 6px bar
				local barHit = Create("Frame",{
					Position=UDim2.new(0,18,0.5,-10), Size=UDim2.new(1,-50,0,20),
					BackgroundTransparency=1, ZIndex=zi()+3, Parent=row,
				})
				local bar = Create("Frame",{
					AnchorPoint=Vector2.new(0,0.5), Position=UDim2.new(0,0,0.5,0),
					Size=UDim2.new(1,0,0,6),
					BackgroundColor3=theme.Stroke, ZIndex=zi()+3, Parent=barHit,
				},{Create("UICorner",{CornerRadius=UDim.new(1,0)})})
				local fill = Create("Frame",{
				...
				barHit.InputBegan:Connect(function(inp)
					if inp.UserInputType==Enum.UserInputType.MouseButton1 or
					   inp.UserInputType==Enum.UserInputType.Touch then
						setRaw((inp.Position.X-barHit.AbsolutePosition.X)/barHit.AbsoluteSize.X)
						beginDrag(function(mi)
							setRaw((mi.Position.X-barHit.AbsolutePosition.X)/barHit.AbsoluteSize.X)
						end, function() end)
					end
				end)
				return {
			...
			local cpOpenH = mobile and 200 or 168
			head.MouseButton1Click:Connect(function()
				open = not open
				Tween(holder,{Size=UDim2.new(1,0,0,open and cpOpenH or 38)},0.18)
			end)
		...
		-- ── Spinner ──────────────────────────────────────────────────────
		-- Pure-Lua spinner — no external assets, works on mobile + PC.
		-- Three dots orbit a ring; no ImageLabel, no hardcoded asset ID.
		function Tab:CreateSpinner(opts)
			...
			local DOTS_N   = 3
			local RADIUS   = 10   -- pixels from centre
			local DOT_SIZE = 5
			local dots = {}
			for i = 1, DOTS_N do
				local d = Create("Frame",{
					...
					BackgroundTransparency = (i-1) * (0.55 / DOTS_N),
			...
			local function startSpin()
				...
				spinConn = RunService.Heartbeat:Connect(function(dt)
					...
					for i, d in ipairs(dots) do
						local a = math.rad(angle + (i-1) * (360/DOTS_N))
						d.Position = UDim2.fromOffset(
							14 + RADIUS * math.cos(a) - DOT_SIZE/2,
							14 + RADIUS * math.sin(a) - DOT_SIZE/2)
					end
				end)
				track(spinConn)
			end
		...
		function Tab:CreateChipGroup(opts)
			...
			-- Use a horizontal UIListLayout with AutomaticSize.X chips so they
			-- wrap naturally.  UIGridLayout with CellSize(0,0) is broken on some
			-- Roblox clients and produces zero-width cells.
			...
			for _, opt in ipairs(options) do
				local sel = selected[opt]==true
				local chipH = mobile and 32 or 26
				local chip = Create("TextButton",{
					Text=" "..tostring(opt).." ",
					Font=Enum.Font.GothamMedium, TextSize=mobile and 13 or 12,
					TextColor3=sel and theme.Accent or theme.SubText,
					BackgroundColor3=sel and theme.AccentBg or theme.Stroke,
					AutomaticSize=Enum.AutomaticSize.X,
					Size=UDim2.new(0,0,0,chipH), ZIndex=zi()+2, Parent=chipRow,
				},{
					Create("UICorner",{CornerRadius=UDim.new(1,0)}),
		...
		function Tab:CreateAccordion(opts)
			...
			-- Wait up to 10 frames for AutomaticSize to settle, then read height.
			-- task.defer alone fires before the layout engine runs on the same frame,
			-- causing AbsoluteSize to read as 0.  We retry until it's non-zero.
			local contentH = 0
			local function waitForHeight(cb)
				task.spawn(function()
					for _ = 1, 10 do
			...
			local function toggle()
				...
				if isOpen then
					waitForHeight(function(h)
						Tween(holder,{Size=UDim2.new(1,0,0,h)},0.22,Enum.EasingStyle.Quint)
					end)
				else
			...
			if isOpen then
				chevron.Rotation = 90
				waitForHeight(function(h) holder.Size = UDim2.new(1,0,0,h) end)
			end
		...
		function Tab:CreateTabGroup(opts)
			...
			-- Content area: ClipsDescendants + a fixed-size switcher.
			-- AutomaticSize.Y is intentionally NOT set here — each page is
			-- individually Visible=false so only the active page contributes
			-- to the layout.  Without this, all pages stack their heights.
		...
		function Tab:CreateSkeleton(opts)
			...
			for i=1,lineCount do
				...
				-- Bounded shimmer: offset sweeps -0.5 → 1.5 over 1.8 s, wraps cleanly.
				local period  = 1.8
				local phaseOff = i * 0.22  -- stagger each line
				local shimConn = RunService.Heartbeat:Connect(function()
					local t = ((tick() + phaseOff) % period) / period  -- 0→1
		...
		-- opts: Name, Steps={string,...}, DefaultStep (1-based index),
		--       Flag, Callback, Info
		function Tab:CreateStepSlider(opts)
			...
			local holder = Create("Frame",{
				BackgroundColor3=theme.Elevated, Size=UDim2.new(1,0,0,mobile and 64 or 58),
				ZIndex=z, Parent=Page,
			},{
				Create("UICorner",{CornerRadius=UDim.new(0,12)}),
				Create("UIStroke",{Color=theme.Stroke,Thickness=1}),
			})
			TopHighlight(holder, z)
			Create("TextLabel",{
				Text=opts.Name or "Step", Font=Enum.Font.GothamMedium, TextSize=13,
				TextColor3=theme.Text, TextXAlignment=Enum.TextXAlignment.Left,
				BackgroundTransparency=1, Position=UDim2.new(0,14,0,8),
				Size=UDim2.new(0.6,0,0,18), ZIndex=z+1, Parent=holder,
			})
			Tooltip(holder, theme, opts.Info)

			local valLbl = Create("TextLabel",{
				Text=steps[cur], Font=Enum.Font.GothamBold, TextSize=12,
				TextColor3=theme.Accent, TextXAlignment=Enum.TextXAlignment.Right,
				BackgroundTransparency=1, Position=UDim2.new(0,14,0,8),
				Size=UDim2.new(1,-28,0,18), ZIndex=z+1, Parent=holder,
			})

			-- Wide invisible hit zone above the visual track
			local trackHit = Create("Frame",{
				Position=UDim2.new(0,14,0,32), Size=UDim2.new(1,-28,0,mobile and 22 or 16),
				BackgroundTransparency=1, ZIndex=z+1, Parent=holder,
			})
			local track = Create("Frame",{
				AnchorPoint=Vector2.new(0,0.5), Position=UDim2.new(0,0,0.5,0),
				Size=UDim2.new(1,0,0,4), BackgroundColor3=theme.Stroke,
				ZIndex=z+1, Parent=trackHit,
			},{Create("UICorner",{CornerRadius=UDim.new(1,0)})})
			local fill = Create("Frame",{
				Size=UDim2.new((cur-1)/math.max(nSteps-1,1),0,1,0),
				BackgroundColor3=theme.Accent, ZIndex=z+2, Parent=track,
			},{Create("UICorner",{CornerRadius=UDim.new(1,0)})})
			...
			local dots = {}
			for i = 1, nSteps do
				local a = (i-1) / math.max(nSteps-1, 1)
				local d = Create("Frame",{
					AnchorPoint=Vector2.new(0.5,0.5),
			...
			local thumb = Create("Frame",{
				AnchorPoint=Vector2.new(0.5,0.5),
				Position=UDim2.new((cur-1)/math.max(nSteps-1,1),0,0.5,0),
				Size=UDim2.fromOffset(mobile and 22 or 18, mobile and 22 or 18),
				BackgroundColor3=Color3.new(1,1,1), ZIndex=z+4, Parent=track,
			},{
				Create("UICorner",{CornerRadius=UDim.new(1,0)}),
			...
			local function setCur(i)
				...
				local a = (cur-1) / math.max(nSteps-1, 1)
				Tween(fill,  {Size=UDim2.new(a,0,1,0)}, 0.15)
				Tween(thumb, {Position=UDim2.new(a,0,0.5,0)}, 0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
				for j, d in ipairs(dots) do
					Tween(d, {BackgroundColor3=j<=cur and theme.Accent or theme.Stroke}, 0.12)
				end
			...
			trackHit.InputBegan:Connect(function(inp)
				if inp.UserInputType==Enum.UserInputType.MouseButton1 or
				   inp.UserInputType==Enum.UserInputType.Touch then
					local a = math.clamp((inp.Position.X-trackHit.AbsolutePosition.X)/trackHit.AbsoluteSize.X,0,1)
					setCur(math.floor(a*(nSteps-1)+0.5)+1)
					beginDrag(function(mi)
						local aa = math.clamp((mi.Position.X-trackHit.AbsolutePosition.X)/trackHit.AbsoluteSize.X,0,1)
						setCur(math.floor(aa*(nSteps-1)+0.5)+1)
					end, function() end)
				end
			end)
			...
			return {
				Set=function(i) setCur(i) end,
				Get=function() return cur, steps[cur] end,
			}
		end

		-- ── Timeline (vertical event log) ─────────────────────────────────
		-- opts: Title, Events={{Label, Time, Type, Detail?},...}
		-- Type: "Success"|"Danger"|"Warning"|"Info" or nil (accent)
		function Tab:CreateTimeline(opts)
			...
			if opts.Title then
				Create("TextLabel",{
					Text=opts.Title, Font=Enum.Font.GothamBold, TextSize=12,
					TextColor3=theme.Accent, TextXAlignment=Enum.TextXAlignment.Left,
					BackgroundTransparency=1, Size=UDim2.new(1,0,0,22), ZIndex=z+1, Parent=holder,
				})
			end

			local function typeColor(tp)
				if tp=="Success" then return theme.Success
				elseif tp=="Danger"  then return theme.Danger
				elseif tp=="Warning" then return theme.Warning
				elseif tp=="Info"    then return theme.Info
				else return theme.Accent end
			end

			for i, ev in ipairs(events) do
				local col    = typeColor(ev.Type)
				local rowH   = ev.Detail and (mobile and 60 or 52) or (mobile and 44 or 36)
				local row    = Create("Frame",{
					BackgroundTransparency=1, Size=UDim2.new(1,0,0,rowH), ZIndex=z+1, Parent=holder,
				})
				-- Vertical stem line (not on last event)
				if i < #events then
					Create("Frame",{
						Position=UDim2.new(0,6,0,18), Size=UDim2.new(0,2,1,-18),
						BackgroundColor3=theme.Stroke, BorderSizePixel=0, ZIndex=z+2, Parent=row,
					})
				end
				-- Event dot
				Create("Frame",{
					AnchorPoint=Vector2.new(0.5,0), Position=UDim2.new(0,7,0,6),
					Size=UDim2.fromOffset(14,14), BackgroundColor3=col,
					ZIndex=z+3, Parent=row,
				},{Create("UICorner",{CornerRadius=UDim.new(1,0)})})
				-- Label
				Create("TextLabel",{
					Text=ev.Label or "Event", Font=Enum.Font.GothamMedium, TextSize=13,
					TextColor3=theme.Text, TextXAlignment=Enum.TextXAlignment.Left,
				...
				if ev.Time then
					Create("TextLabel",{
						Text=tostring(ev.Time), Font=Enum.Font.Gotham, TextSize=11,
						TextColor3=theme.SubText, TextXAlignment=Enum.TextXAlignment.Right,
						BackgroundTransparency=1, AnchorPoint=Vector2.new(1,0),
						Position=UDim2.new(1,0,0,4), Size=UDim2.new(0,70,0,18),
						ZIndex=z+3, Parent=row,
					})
				end
				...
				if ev.Detail then
					Create("TextLabel",{
						Text=ev.Detail, Font=Enum.Font.Gotham, TextSize=11,
						TextColor3=theme.SubText, TextWrapped=true,
						TextXAlignment=Enum.TextXAlignment.Left,
		...
		-- ── List (scrollable item list with icons + row select) ───────────
		-- opts: Name, Items={{Label,Icon?,Value?,Callback?},...},
		--       MaxVisible, OnSelect
		function Tab:CreateList(opts)
			...
			local rowH    = mobile and 42 or 34
			local listH   = math.min(#items, maxVis) * rowH

			local frame = Create("Frame",{
				BackgroundColor3=theme.Elevated,
				Size=UDim2.new(1,0,0,(opts.Name and listH+28 or listH)),
			...
			if opts.Name then
				...
				local hdr = Create("Frame",{
					BackgroundColor3=theme.Secondary,
					Size=UDim2.new(1,0,0,headerH), ZIndex=z+1, Parent=frame,
				},{Create("UICorner",{CornerRadius=UDim.new(0,10)})})
				...
				Create("TextLabel",{
					Text=opts.Name, Font=Enum.Font.GothamBold, TextSize=11,
					TextColor3=theme.SubText, TextXAlignment=Enum.TextXAlignment.Left,
					BackgroundTransparency=1, Position=UDim2.new(0,12,0,0),
					Size=UDim2.new(1,-12,1,0), ZIndex=z+2, Parent=hdr,
				})
			end
			...
			Create("UIListLayout",{
				Padding=UDim.new(0,0), SortOrder=Enum.SortOrder.LayoutOrder, Parent=scroll,
			})
			...
			for i, item in ipairs(items) do
				...
				if item.Icon then
					Create("ImageLabel",{
						Image=item.Icon, BackgroundTransparency=1, ImageColor3=theme.SubText,
						Position=UDim2.new(0,10,0.5,-8), Size=UDim2.fromOffset(16,16),
						ZIndex=z+3, Parent=row,
					})
				end
				Create("TextLabel",{
					Text=item.Label or tostring(i), Font=Enum.Font.Gotham, TextSize=13,
					TextColor3=theme.Text, TextXAlignment=Enum.TextXAlignment.Left,
					BackgroundTransparency=1,
				...
				if item.Value then
					Create("TextLabel",{
						Text=tostring(item.Value), Font=Enum.Font.Gotham, TextSize=11,
						TextColor3=theme.SubText, TextXAlignment=Enum.TextXAlignment.Right,
						BackgroundTransparency=1, AnchorPoint=Vector2.new(1,0.5),
						Position=UDim2.new(1,-12,0.5,0), Size=UDim2.new(0,80,0,18),
						ZIndex=z+3, Parent=row,
					})
				end
				...
				if i < #items then
					Create("Frame",{
						AnchorPoint=Vector2.new(0,1), Position=UDim2.new(0,0,1,0),
						Size=UDim2.new(1,0,0,1), BackgroundColor3=theme.Stroke,
						BorderSizePixel=0, ZIndex=z+3, Parent=row,
					})
				end
				local base = row.BackgroundColor3
				row.MouseEnter:Connect(function()
					if selected~=i then Tween(row,{BackgroundColor3=theme.ElevatedHover},0.08) end
				end)
				row.MouseLeave:Connect(function()
					if selected~=i then Tween(row,{BackgroundColor3=base},0.12) end
				end)
				row.MouseButton1Click:Connect(function()
					if selected and rows[selected] then
						local si = selected
						Tween(rows[si],{BackgroundColor3=si%2==0 and theme.Background or theme.Elevated},0.12)
					end
					selected = i
					Tween(row,{BackgroundColor3=theme.AccentBg},0.12)
		...
		-- ── Banner / hero card ────────────────────────────────────────────
		-- opts: Title, Subtitle, Image (optional bg), Height, Callback
		function Tab:CreateBanner(opts)
			opts = opts or {}
			local z    = zi()
			local h    = opts.Height or (mobile and 80 or 90)
			local bgC  = opts.AccentColor or theme.AccentBg

			local frame = Create("Frame",{
				BackgroundColor3=bgC, Size=UDim2.new(1,0,0,h),
				ClipsDescendants=true, ZIndex=z, Parent=Page,
			},{
				Create("UICorner",{CornerRadius=UDim.new(0,14)}),
				Create("UIStroke",{Color=Darken(bgC,0.15), Thickness=1}),
			})

			-- Optional background image
			if opts.Image then
				Create("ImageLabel",{
					Image=opts.Image, BackgroundTransparency=1,
					Size=UDim2.new(1,0,1,0), ScaleType=Enum.ScaleType.Crop,
					ImageTransparency=0.38, ZIndex=z+1, Parent=frame,
				})
			end
			...
			if opts.Title then
				Create("TextLabel",{
					Text=opts.Title, Font=Enum.Font.GothamBold,
					TextSize=mobile and 15 or 16,
					TextColor3=Color3.new(1,1,1), TextXAlignment=Enum.TextXAlignment.Left,
					BackgroundTransparency=1,
					Position=UDim2.new(0,16,0.5,-12), Size=UDim2.new(1,-32,0,20),
					ZIndex=z+3, Parent=frame,
				})
			end
			if opts.Subtitle then
				Create("TextLabel",{
					Text=opts.Subtitle, Font=Enum.Font.Gotham,
					TextSize=mobile and 11 or 12,
					TextColor3=Color3.fromRGB(210,210,220),
					TextXAlignment=Enum.TextXAlignment.Left,
					BackgroundTransparency=1,
					Position=UDim2.new(0,16,0.5,10), Size=UDim2.new(1,-32,0,16),
					ZIndex=z+3, Parent=frame,
				})
			end
		...
		-- ── Code block ────────────────────────────────────────────────────
		-- opts: Code (string), Language (label), Height, OnCopy
		function Tab:CreateCodeBlock(opts)
			opts = opts or {}
			local z = zi()
			local h = opts.Height or (mobile and 80 or 100)

			local frame = Create("Frame",{
				BackgroundColor3=Darken(theme.Background, 0.06),
			...
			if opts.Language then
				Create("TextLabel",{
					Text=opts.Language:upper(), Font=Enum.Font.Code, TextSize=10,
			...
			local scroll = Create("ScrollingFrame",{
				...
				CanvasSize=UDim2.new(0,0,0,0), AutomaticCanvasSize=Enum.AutomaticSize.Y,
				ScrollBarThickness=mobile and 4 or 2,
				ScrollBarImageColor3=theme.Stroke,
				ScrollingDirection=Enum.ScrollingDirection.Y,
				BorderSizePixel=0, ZIndex=z+1, Parent=frame,
			})
			Create("TextLabel",{
				Text=opts.Code or "", Font=Enum.Font.Code,
				TextSize=mobile and 11 or 12,
				TextColor3=theme.Text, TextWrapped=false,
				TextXAlignment=Enum.TextXAlignment.Left,
				TextYAlignment=Enum.TextYAlignment.Top,
				BackgroundTransparency=1, AutomaticSize=Enum.AutomaticSize.Y,
				Size=UDim2.new(1,0,0,0), ZIndex=z+2, Parent=scroll,
			})

			return {
				SetCode = function(code)
					local lbl = scroll:FindFirstChildOfClass("TextLabel")
					if lbl then lbl.Text = code end
				end,
			}
		end
...
-- Apply a theme to every open window at once.
function NovaUI:SetGlobalTheme(nameOrTable)
	for _, w in ipairs(NovaUI.Windows) do w:SetTheme(nameOrTable) end
end
